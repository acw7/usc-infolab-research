# This script searches many H3 A-to-B pairs in one parquet sample and maps a selected pair.
# It computes route probabilities with time and distance thresholds for exploratory analysis.
...
def build_user_sequences_with_time(
    ...
    consecutive duplicates. Returns compact per-user arrays.
    """
    df = df[[USER_COL, TIME_COL, LAT_COL, LON_COL]].copy()
    df[LAT_COL] = pd.to_numeric(df[LAT_COL], errors="coerce")
    df[LON_COL] = pd.to_numeric(df[LON_COL], errors="coerce")
    df[TIME_COL] = pd.to_numeric(df[TIME_COL], errors="coerce")
    df = df.dropna(subset=[USER_COL, TIME_COL, LAT_COL, LON_COL])
    if df.empty:
        return []

    df["_user_code"] = pd.factorize(df[USER_COL], sort=False)[0].astype(np.int64)
    df = df.sort_values(["_user_code", TIME_COL], kind="mergesort").reset_index(drop=True)

    df["cell"] = np.fromiter(
        (
            h3.latlng_to_cell(float(lat), float(lon), res)
            for lat, lon in df[[LAT_COL, LON_COL]].itertuples(index=False, name=None)
        ),
        dtype=object,
        count=len(df),
    )

    unique_hex_counts = df.groupby("_user_code", sort=False, observed=True)["cell"].nunique()
    eligible_users = unique_hex_counts.index[unique_hex_counts > min_unique_hexes_per_user]
    if len(eligible_users) == 0:
        return []

    df = df.loc[
        df["_user_code"].isin(eligible_users),
        ["_user_code", TIME_COL, "cell"],
    ].copy()
    if df.empty:
        return []
    ...
    user_codes = df["_user_code"].to_numpy(dtype=np.int64, copy=False)
    timestamps = df[TIME_COL].to_numpy(dtype=np.float64, copy=False)
    cells = df["cell"].to_numpy(dtype=object, copy=False)

    keep_mask = np.empty(len(df), dtype=bool)
    keep_mask[0] = True
    keep_mask[1:] = (user_codes[1:] != user_codes[:-1]) | (cells[1:] != cells[:-1])
    user_codes = user_codes[keep_mask]
    ...
    split_points = np.flatnonzero(user_codes[1:] != user_codes[:-1]) + 1
    boundaries = np.concatenate(([0], split_points, [len(user_codes)]))
...
def find_all_ab_trips(
    sequences: list[UserSequence],
    time_threshold: float,
    max_length: int = MAX_TRIP_LENGTH,
    ...
    for sequence in sequences:
        ...
        for start_idx in range(num_cells - 1):
            hex_a = str(cells[start_idx])
            seen = {hex_a}
            stop_idx = min(
                num_cells,
                start_idx + max_length,
                int(searchsorted(timestamps, timestamps[start_idx] + time_threshold, side="right")),
            )
            for end_idx in range(start_idx + 1, stop_idx):
                hex_b = str(cells[end_idx])
                if hex_b in seen:
                    break

                seen.add(hex_b)
                if haversine(hex_a, hex_b) < MIN_KM_DIST:
                    continue

                pair_trips = all_trips[(hex_a, hex_b)]
                if len(pair_trips) >= MAX_TRIPS_PER_PAIR:
                    continue

                pair_trips.append(cells[start_idx : end_idx + 1].tolist())
...
def main():
    ...
    sequences = build_user_sequences_with_time(df, RES, MIN_UNIQUE_HEXES_PER_USER)
